...
if selectedCharacter then
    TriggerServerEvent("Ayse_Properties:getProperties")
end
...
RegisterNetEvent("Ayse_Doorlocks:returnDoors", function()
    TriggerServerEvent("Ayse_Properties:getProperties")
end)

RegisterNetEvent("Ayse_Properties:returnProperties", function(ownedProperties)
    exports["Ayse_Doorlocks"]:doorsResetDefault()
    ...
    for _, property in pairs(properties) do
        for _, doorInfo in pairs(property.doors) do
            ...
            Wait(50)
            exports["Ayse_Doorlocks"]:doorAdd(doorInfo)
        end
    end
end)

RegisterNetEvent("Ayse_Properties:refresh", function()
    TriggerServerEvent("Ayse_Properties:getProperties")
end)

RegisterNetEvent("Ayse_Properties:updateDoors", function(updatedDoors)
...
RegisterNetEvent("Ayse_Properties:updateUI", function(id, player)
    for _, property in pairs(properties) do
...
RegisterNetEvent("Ayse_Properties:returnOwnedProperties", function(result, players)
...
RegisterNetEvent("Ayse_Properties:refreshAccess", function(result, players)
    local owned = manage(result, players)
    ...
    for _, property in pairs(properties) do
        if property.propertyid == id then
            if money >= property.price then
                ...
                TriggerServerEvent("Ayse_Properties:purchaseProperty", property.propertyid)
            else
                SendNUIMessage({
                    type = "purchase",
                    success = false
...
RegisterNUICallback("manage", function(data)
    TriggerServerEvent("Ayse_Properties:getOwnedProperties")
end)

RegisterNUICallback("grantAccess", function(data)
    TriggerServerEvent("Ayse_Properties:grantAccess", data.id, data.property)
end)

RegisterNUICallback("removeAccess", function(data)
    TriggerServerEvent("Ayse_Properties:removeAccess", data.character, data.property)
end)